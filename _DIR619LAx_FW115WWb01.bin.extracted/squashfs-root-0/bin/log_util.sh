...
LOG_SMTP_CONFIG="/tmp/smtp_subject.log"
...
if [ "$1" != "" ]; then
	if [ "$SMTP_AUTH_ENABLED" = 1 ]; then
		if [ "$SMTP_SECRET" = 1 ]; then
			email -W $LOG_SMTP_CONFIG -f $LOG_SEND_FROM -n "mydlink" -s  "$EMAIL_SUBJECT" -r "$SMTP_SERVER" -z "/tmp/Gatewaylog.txt" -p $SMTP_PORT -tls -m login -u $SMTP_USER_NAME -i $SMTP_PASSWORD $LOG_SEND_TO &
		else		
			email -W $LOG_SMTP_CONFIG -f $LOG_SEND_FROM -n "mydlink" -s  "$EMAIL_SUBJECT" -r "$SMTP_SERVER" -z "/tmp/Gatewaylog.txt" -p $SMTP_PORT -m login -u $SMTP_USER_NAME -i $SMTP_PASSWORD $LOG_SEND_TO &
		fi
	else
		email -W $LOG_SMTP_CONFIG -f $LOG_SEND_FROM -n "mydlink" -s  "$EMAIL_SUBJECT" -r "$SMTP_SERVER" -z "/tmp/Gatewaylog.txt" -p $SMTP_PORT $LOG_SEND_TO &
	fi
#	if [ $1 -ge 32 ]; then
