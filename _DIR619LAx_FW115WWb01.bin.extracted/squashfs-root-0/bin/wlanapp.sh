...
for WLAN in $WLAN_INTERFACE ; do
	...
	if [ "$ENCRYPT" -lt 2 ]; then
		...
		eval `$GETMIB MAC_AUTH_ENABLED1`
		eval `$GETMIB MAC_AUTH_ENABLED2`		
#		if [ "$ENABLE_1X" != 0 ] || [ "$MAC_AUTH_ENABLED1" != 0 ] || [ "$MAC_AUTH_ENABLED2" != 0 ]; then
		if [ "$ENABLE_1X" != 0 ]; then
			_ENABLE_1X=1
			_USE_RS=1
		fi
...
###############################################################
	...
	if [ "$VAP" = "va" ]; then
		...
		if [ "$VAP" != "vx" ]; then
			...
			if [ $ROOT_AUTH_ENABLE = 1 ] || [ $WSC_DISABLE = 0 ]; then
				VALID_WLAN_INTERFACE="$VALID_WLAN_INTERFACE $WLAN"
			fi
		fi
	fi	
