#for dlink:websGetVar
import angr
import re
from kingcore.color import Color
#p = angr.Project('../tests/dir619l_a115b01', load_options={'auto_load_libs': False})
...
def getformfunctionstrings_mips(p, func):
	ans = []
	gp = getgp(func)
	targets = findaddrfromins(func, 'lw', '$a1, ')
	for target in targets:
		seta0 = getinsfromaddr(func, target)
		if('$gp' not in seta0):
			continue
		left = seta0.find('lw $a1, ')
		right = seta0.find('(')
		...
		for i in range(1,7):
		  ...
		  if(tmpins == None):
		    continue
		  if('addiu $a1, $a1, ' in tmpins):        
		    adda1 = target + i * 4
		    ins = getinsfromaddr(func, adda1)
		    tmp = ins.find('addiu $a1, $a1, ')
		    ...
		    try:
		      ...
		      continue
		    if((onestring != None) and ('.' not in onestring) and ('\n' not in onestring) and ('/' not in onestring) and ('%' not in onestring) and (onestring != 'true') and (onestring.isdigit() == False) and (len(onestring) > 1)):
