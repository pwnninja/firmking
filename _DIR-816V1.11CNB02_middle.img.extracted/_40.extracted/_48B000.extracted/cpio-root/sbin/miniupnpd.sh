...
if [ "$1" == "init" ]; then
	...
	PORT=6352
	INTERVAL=`nvram_get 2860 upnptimeInterval`
	TTL=`nvram_get 2860 upnpTTL`
	if [ "$INTERVAL" = "" -o "$TTL" = "" ]; then
		echo "miniupnp : Invalid INTERVAL or TTL !"
		INTERVAL=20
		TTL=4
	fi
...
SERIAL_NUMBER=`nvram_get 2860 UPNPSerialNumber`
if [ -n "$SERIAL_NUMBER" ]; then 
	TMPSERIAL=$SERIAL_NUMBER
else
	TMPSERIAL=104
fi
