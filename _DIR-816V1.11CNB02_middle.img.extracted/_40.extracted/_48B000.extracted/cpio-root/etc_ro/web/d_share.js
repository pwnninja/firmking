...
function Lan1EqLan2(ip1, mask1, ip2, mask2)
{
	d11 = getDigit(ip1,1);
	d12 = getDigit(mask1, 1);
	d21 = getDigit(ip2,1);
	d22 = getDigit(mask2,1);
	d1 = d11 & d12;
	d2 = d21 & d22;
	if (d1 != d2)
		return false;
	...
	d11 = getDigit(ip1,2);
	d12 = getDigit(mask1, 2);
	d21 = getDigit(ip2,2);
	d22 = getDigit(mask2,2);
	d1 = d11 & d12;
	d2 = d21 & d22;
	if (d1 != d2)
		return false;
	...
	d11 = getDigit(ip1,3);
	d12 = getDigit(mask1, 3);
	d21 = getDigit(ip2,3);
	d22 = getDigit(mask2,3);
	d1 = d11 & d12;
	d2 = d21 & d22;
	if (d1 != d2)
		return false;
	...
	d11 = getDigit(ip1,4);
	d12 = getDigit(mask1, 4);
	d21 = getDigit(ip2,4);
	d22 = getDigit(mask2,4);
	d1 = d11 & d12;
	d2 = d21 & d22;
	if (d1 != d2)
		return false;
...
{
	if (ip.value=="") {
		alert("IP地址不能为空. 必须为 xxx.xxx.xxx.xxx形式的数字串.");
	...
	if(count!=3)
	{
		alert("IP地址无效. 必须为 xxx.xxx.xxx.xxx形式的数字串.");
		ip.focus();
		return false;
	}
	...
	if ( !checkDigitRange(ip.value,2,0,255) ) {
		alert('IP地址第二个数字段必须为 0-255.');
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
	if ( !checkDigitRange(ip.value,3,0,255) ) {
		alert('IP地址第三个数字段必须为 0-255.');
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
	if ( !checkDigitRange(ip.value,4,1,254) ) {
		alert('IP地址第四个数字段必须为 1-254.');
		ip.value = ip.defaultValue;
		ip.focus();
		return false;
	}
...
function checkIPNotZeroHeaded(ip)
{
	if (ip.value=="") {
		alert("IP地址不能为空. 必须为 xxx.xxx.xxx.xxx形式的数字串.");
		ip.value = ip.defaultValue;
...
function ipaddrCompare(ip1, ip2)
{
	...
	ip1_3 = getDigit(ip1,3);
	ip1_4 = getDigit(ip1,4);
	ipaddr1 = ip1_1*256*256*256 + ip1_2*256*256 + ip1_3*256 + ip1_4;

	ip2_1 = getDigit(ip2,1);
	ip2_2 = getDigit(ip2,2);
	ip2_3 = getDigit(ip2,3);
	ip2_4 = getDigit(ip2,4);
	ipaddr2 = ip2_1*256*256*256 + ip2_2*256*256 + ip2_3*256 + ip2_4;
...
// confirm that destination ip is not network
function checkIpNetwork(netip,netmask)
{
	var ip = netip.value;
...
{
 	 var str = new Array("", "第一个", "地二个", "第三个", "第四个");
	var i, d;
	var s,m;
	if (netmask.value=="") {
		alert("子网掩码无效. 必须为 xxx.xxx.xxx.xxx形式的数字串.");
	...
	for (i=1; i<=4; i++) 
	{
		...
		if( !(d==0 || d==128 || d==192 || d==224 || d==240 || d==248 || d==252 || d==254 || d==255 )) {
			alert('子网掩码的 '+str[i]+' 字段无效.\n必须是0, 128, 192, 224, 240, 248, 252 或254');
			netmask.focus();
			return false;
		}
		if(d!=255)
...
{
	 var str = new Array("", "第一个", "第二个", "第三个", "第四个");
	 var str2=netmask.value;
	 var i, d;
	  var s,m;
	  var count=0;
	...
	{	
		alert("子网掩码不能为0.0.0.0.");
		netmask.focus();
		return false;
	}
	if((d1==255)&&(d2==255)&&(d3==255)&&(d4==255))
	{	
		alert("子网掩码不能为255.255.255.255.");
		netmask.focus();
		return false;
	}
...
function checkFormatUnicastMac(macaddr)
{
	...
	{
		alert("MAC地址无效. 必须为 xx:xx:xx:xx:xx:xx形式的数字串.");
		macaddr.focus();
		return false;
	}
	...
	for (var i=0; i<str.length; i++) 
	{
		...
		alert("MAC地址无效. 必须为 xx:xx:xx:xx:xx:xx形式的数字串(x代表0-9或a-f的十六进制数字)");
		macaddr.focus();
		return false;
	}
...
// shenhuiye 20121121: fix bug that multicast/broadcast mac such as FF:FF:FF:FF:FF:FF is allowed
	//if(str.charAt(1)&1)
	if(parseInt(str.charAt(1), 16)&1)
	{
		alert("无效的广播/多播MAC地址!");
		macaddr.focus();
		return false;
...
function checkMac(macaddr)
{
	...
	if(str == "00:00:00:00:00:00")
	{
	        alert("MAC地址不能为00:00:00:00:00:00'");
		return false;
	}
	var count=0;
	for (var i=0; i<str.length; i++) 
...
function isSameMac(mac1,mac2)
{
	for(var i = 0; i < 6 ; i ++)
	{
		if(getDigitmac(mac1,i+1) != getDigitmac(mac2,i+1))
			return false;
	}

	return true;
}
function getDigitmac(str, num)
{
  var i=1;
  if ( num != 1 ) {
  	while (i!=num && str.length!=0) {
...
function checkMulticastMAC(field)
{

   if ( (getDigitmac(field.value,1)==1)&&(getDigitmac(field.value,2)==0 )&&( getDigitmac(field.value,3)==94 ) )
   {
		alert("<#alert_35#>");
...
function checkCloneMac(field)
{
	...
	if ( re_f.test(field.value) || re_z.test(field.value) ){
		alert("<#alert_38#>");
		field.focus();
		return false;
	}

	if(!checkMulticastMAC(field))
		return false;
...
//stephen: add innerText for all Browers
function isIE()
{ //ie?
